...
class TestData(Dataset):
    def __init__(self, root):
        ...
        #self.num_frame = min(self.num_frame, 3)  # debug
        ...
        for i in range(1, self.num_frame+1):
            ...
            img_i = cv2.imread(os.path.join(root, f"{i}.png"))
            mask_i = cv2.imread(os.path.join(root, f"{i}_mask.png"), 0)
            mask_i = remove_noise(mask_i)
            mask_i = (mask_i > 50)
            slic_i = segmentation.slic(img_i, n_segments=60, mask=mask_i, start_label=1, min_size_factor=0.5,
                                       enforce_connectivity=False, compactness=20, max_iter=200, sigma=0)
            slic_i -= 1
            # img_slic = segmentation.mark_boundaries(img_i, slic_i)
            # cv2.imshow("img", img_slic)
            # cv2.waitKey()
            # cv2.destroyAllWindows()
            src_pixel_group_i = sample_from_slic(slic_i, group_size=60)
...
def affinity2seg(A_in, slic_in, img1_np, mask):
    pred_G_i = sync_motion_seg(A_in.unsqueeze(0), cut_thres=0.01).squeeze(0)
    pred_G_i = pred_G_i / (pred_G_i.sum(-1, keepdim=True) + 1e-6)

    #pred_G_i = filter_superpixel_seg(slic_in, img1_np, pred_G_i)
...
def select(model, test_dataset, test_loader, output_folder):
    parts_database = []
    pred_corr_all = {}
    for data_pack in tqdm(test_loader):
        ...
        for i in range(len(img_1)):
            ...
            seg_pred_f_np = affinity2seg(torch.sigmoid(res["A_f"][i]), res["slic_f"][i], img_1_np, mask_1[i])
            ...
            pred_corr_all[f"{pair[0]}{pair[1]}"] = res["pred_corr"][i, ..., 2:].to("cpu").numpy()
            seg_pred_b_np = affinity2seg(torch.sigmoid(res["A_b"][i]), res["slic_b"][i], img_2_np, mask_2[i])
    ...
    paths = selector.select(test_dataset.slic_list, filter_thrd=0.4, debug_folder=output_folder)
...
def main(args):
    ...
    for char_name in char_name_list:  # Clean, PantBoy2, SkirtGirl2, Ostrich
        print(char_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--init_weight_path', default="../checkpoints/train_cluster/model_best.pth.tar", type=str)
    parser.add_argument('--test_folder', default='/mnt/neghvar/mnt/DATA_LINUX/zhan/real_sheets/',
                        type=str, help='folder of testing data')
    parser.add_argument('--output_folder', default='/mnt/neghvar/mnt/DATA_LINUX/zhan/puppets/results/train_cluster_real/',
